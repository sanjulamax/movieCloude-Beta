...
import noResults from '../assets/search_no_result.png'
...
const Home = ()=>{
    ...
    return ( 
        ...
        <div className='   flex flex-row bg-opacity-10 bg-white rounded-lg border-l-2 border-b-2 border-r-2 border-rose-500 p-6 mb-2 backdrop-blur-lg' >
            <div className='flex flex-row  w-1/2'>

            <input className=' max-[430px]:h-5/ max-[430px]:w-[100px]  border-2  h-10 border-blue-500 rounded-lg p-2 w-2/4 mr-6' type="text" placeholder="Enter Movie Name" value={inpMovie} onChange={(e)=>setInpMovie(e.target.value)} />
          ...
          <a href='/' className=' flex  pl-0 ml-8  w-fit'><img src={logo} alt='logo' className='  max-[430px]:w-4/6 max-[430px]:h-5/6 max-[430px]:flex max-[430px]:justify-start hover:border-4 hover:border-green-500 w-6/6 h-16 ml-10 rounded-xl w-fit' /></a>
   ...
   <div className='grid grid-cols-6 max-[430px]:grid max-[430px]:grid-cols-3 ' >
              {noMovie ?<div className=' border-2 border-white w-screen h-screen flex items-start justify-center '> <div className='border-2 mt-20 border-white backdrop-blur-lg bg-white bg-opacity-10 rounded-lg '><img src={noResults} className='h-96  ' alt="no result found" /></div></div>:mvList.map((item)=>{
                  ...
                  return(
                      <div className='max-[430px]:h-[500px]  h-[600px] pb-6 hover:border-2 hover:border-amber-300  items-center  border-2 border-white p-2 m-2  text-white bg-transparent backdrop-blur-xl rounded-md' key={item.id}>
                            ...
                            {engPlayer?  <a className='absolute hover:bg-red-500  border-none  font-extralight  text-[15px] w-4/6 p-2  rounded-xl left-1/2 top-[95%] transform -translate-x-1/2 -translate-y-1/2 bg-blue-600 max-[430px]:pr-2 max-[430px]:pl-2 max-[430px]:text-[10px] max-[430px]:w-5/6 pl-4 pr-4'  href={`/EngPlayerPage/${item.id}`}>Watch Add Free</a>:null}                      </div>
                  )
              })}

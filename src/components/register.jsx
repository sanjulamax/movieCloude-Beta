...
const Register = ()=>{
    ...
    return (
        <div className=" bg-regBack flex w-screen  h-screen align-middle items-center justify-center">
                <div  id='Regmsg' className={` absolute top-5 border-2 border-red-500 p-2 bg-red-100 rounded-xl   hidden`}><p>Something wen wrong...Check Password Contain at least 6 caharacters or Try again with different Email</p> <button className="ml-3 font-medium text-red-700 " onClick={errCloseFunc}>x</button></div>
            ...
            <div className="flex   flex-col w-[50%] h-full  pt-3 pr-3">
               ...
               <div className="flex flex-row"><label for='password' className=" font-semibold mb-2 mt-3">Password</label>
                ...
                <input required  onChange={(e)=>setPass(e.target.value)} className="border-2 pl-4 border-blue-200  rounded-lg placeholder:text-xs" type="password"  placeholder="Use At Least 6 Characters"></input>
               ...
               <div className="flex flex-row"><label for='confirm password' className="font-semibold mb-2 mt-3">Confirm Password</label>
                ...
                <button disabled={!passMatch} onClick={register} className="bg-green-500 font-mono mt-4 text-white text-lg p-1 pl-6 pr-6 rounded-lg hover:bg-transparent hover:border-2 hover:border-green-500 hover:text-black">Register</button>
